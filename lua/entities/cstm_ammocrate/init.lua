...
function ENT:SpawnFunction( ply, tr )
    ...
    local ent = ents.Create("cstm_ammocrate")
					...
					if self.AmmoCharge > 0 then
						...
						if am != -1 then
							...
							if wep.Primary and wep.Primary.ClipSize then
								...
								if math.Round(wep.Primary.ClipSize * 12 * wep.MaxAmmoMod) + math.Clamp(wep.Primary.ClipSize - mag, 0, wep.Primary.ClipSize) > amc then
									self.AmmoCharge = self.AmmoCharge - 1
									v:EmitSound("items/ammo_pickup.wav", 60, 100)
									
									ammo = math.Clamp(amc + (wep.Primary.ClipSize > 50 and wep.Primary.ClipSize / 2 or wep.Primary.ClipSize) * wep.GiveAmmoMod, 0, math.Round(wep.Primary.ClipSize * 12 * wep.MaxAmmoMod) + math.Clamp(wep.Primary.ClipSize - mag, 0, wep.Primary.ClipSize))
									v:SetAmmo(ammo, am)
								end
							end
						end
						...
						if v:GetAmmoCount("Grenade") < 12 then
