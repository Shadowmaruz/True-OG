...
function PANEL:DoClick()
	...
	local karma = PS.Config.CalculateBuyKarma(LocalPlayer(), self.Data)
	if not LocalPlayer():PS_HasItem(self.Data.ID) then
		if not LocalPlayer():PS_HasPoints(points) and LocalPlayer():PS_HasKarma(karma) then
			notification.AddLegacy("You don't have enough points for this!", NOTIFY_GENERIC, 5)
		elseif not LocalPlayer():PS_HasKarma(karma) and LocalPlayer():PS_HasPoints(points) then
			notification.AddLegacy("You don't have enough karma for this!", NOTIFY_GENERIC, 5)
		elseif not LocalPlayer():PS_HasKarma(karma) and not LocalPlayer():PS_HasPoints(points) then
			notification.AddLegacy("You don't have enough points or karma for this!", NOTIFY_GENERIC, 5)	
		end
	end
	...
	elseif LocalPlayer():PS_HasPoints(points) and LocalPlayer():PS_HasKarma(karma) then
	...
	if LocalPlayer():PS_HasItem(self.Data.ID) then
		...
		if LocalPlayer():PS_HasItemEquipped(self.Data.ID) then
			...
			menu:AddOption('Equip', function()
				if not LocalPlayer():PS_HasKarma(karma) then
					notification.AddLegacy("Your karma is too low to equip this!", NOTIFY_GENERIC, 5)
				else
