...
local CT, clip, mul, ply, wep, sequence, seqdur, activity, bone, matrix, m, AimVec, AvailableReload, AmountToReload, DTInt3, v, model, sprite, pos, ang, drawpos, td, trace, dir, lt, fw, aim, CamData, old, bullet, eyeang, muz, dtint3, par, fov, ScopeStatus, sp, can, wc, cyc
				...
				if (v.type == "Model" and IsValid(model)) then
					...
					if n then
						...
						if not f and dtint3 == 1 then
							...
							scale = 10 / dist
							size = size / scale
							self.DrawTable = {dist = dist - self.RecoilAmt, scale = scale, size = size, dir = dir, mat = mat}
						end
	...
	function SWEP:FireAnimationEvent(pos, ang, ev)
		if ev == 5001 then
			if not self.Owner:ShouldDrawLocalPlayer() then
				vm = self.Owner:GetViewModel()
				muz = vm:GetAttachment(vm:LookupAttachment("1"))
				...
				if not self.dt.Sil then -- I check if it's not nil and then if it's nil to ensure that weapons that don't have a silencer don't bug
				
					if GetConVarNumber("cstm_ef_extramuzzle") > 0 then
						ParticleEffectAttach(self.Muzzle or "cstm_muzzle_pistol", PATTACH_POINT_FOLLOW, vm, vm:LookupAttachment("1"))
					end
					
					if GetConVarNumber("cstm_ef_sparks") > 0 then
						ParticleEffectAttach(self.SparkEffect or "cstm_child_sparks_small", PATTACH_POINT_FOLLOW, vm, vm:LookupAttachment("1"))
					end
					...
					//if GetConVarNumber("cstm_ef_extramuzzle") > 0 then
						//ParticleEffectAttach(self.MuzzleEffect or "cstm_child_muzzle_small", PATTACH_POINT_FOLLOW, vm, vm:LookupAttachment("1"))
						/*local par = self.Em:Add("effects/muzzleflashX", muz.Pos)
						par:SetStartSize(8)
						par:SetEndSize(8)
						par:SetStartAlpha(255)
						par:SetEndAlpha(0)
						par:SetDieTime(0.075)
						par:SetRoll(math.Rand(0, 360))
						par:SetVelocity(self.Owner:GetVelocity())*/
					//end
					
					//if GetConVarNumber("cstm_ef_sparks") > 0 then
						//ParticleEffect(self.SparkEffect or "cstm_child_sparks_pistol", muz.Pos + self.Owner:GetAimVector() * 5, self.Owner:EyeAngles(), muz)
						/*for i = 1, 10 do
							local par = self.Em:Add("effects/fire_embers" .. rand(1, 3), muz.Pos)
							par:SetStartSize(1)
							par:SetEndSize(1)
							par:SetStartAlpha(255)
							par:SetEndAlpha(0)
							par:SetDieTime(0.15)
							par:SetRoll(rand(0, 360))
							par:SetRollDelta(math.Rand(-1, 1))
							par:SetLighting(false)
							par:SetVelocity(self.Owner:GetAimVector() + VectorRand() * 20 + self.Owner:GetVelocity())
						end*/
					//end
				end
