...
if SERVER then
	...
	function SWEP:NWATT2(number, scope)
		...
		if scope then 
			...
			self.FOVMod = CWAttachments[CWAttachmentsMeta[number]].scope.fovmod
		...
		if CWAttachmentsMeta[number] then
			can = true
			
			if CWAttachments[CWAttachmentsMeta[number]].svattfunc then
				CWAttachments[CWAttachmentsMeta[number]].svattfunc(self)
			end
		end
...
if CLIENT then
			...
			if self.VElements then
				...
				if entry and entry.color.a == 255 then
					if self.AimPos == self.ELCANPos then
						return GetConVarNumber("cstm_aim_sensitivity_elcan") * self.Owner:GetFOV() * (1 - add)
					end
				else
	...
	Comp["rail"] = {}
	Comp["aimpoint"] = {"acog", "eotech", "reflex", "riflereflex", "kobra", "ballistic", "elcan"}
	Comp["acog"] = {"aimpoint", "eotech", "reflex", "riflereflex", "kobra", "ballistic", "elcan"}
	Comp["eotech"] = {"acog", "aimpoint", "reflex", "riflereflex", "kobra", "ballistic", "elcan"}
	Comp["reflex"] = {"acog", "aimpoint", "eotech", "riflereflex", "kobra", "ballistic", "elcan"}
	Comp["riflereflex"] = {"acog", "eotech", "reflex", "aimpoint", "kobra", "ballistic", "elcan"}
	Comp["ballistic"] = {"aimpoint", "acog", "eotech", "reflex", "riflereflex", "kobra", "elcan"}
	Comp["elcan"] = {"acog", "aimpoint", "eotech", "reflex", "riflereflex", "kobra", "ballistic"}
	...
	function SWEP:DrawHUDElements(x, y, FT)
		...
		if CH > 0 then
			...
			if (activity == "ACT_VM_RELOAD" or activity == "ACT_VM_RELOAD_EMPTY" or activity == "ACT_VM_RELOAD_SILENCED") and cyc < 0.99 then
				if mag == 0 or self.SkipIdle then
					self.WasZero = true
					
					if not self.NoBoltAnim then
						if self.dt.Sil and self.IncAmmoPercSil then
							if activity == "ACT_VM_RELOAD_EMPTY" and self.NonEmptyReloadAnim then
								...
								draw.BlurredText("RELOADING " .. clamp(math.Round(cyc * 100), 0, 100) .. "%", "BROOP_Big", "BROOP_BigBlur", x - 100, y - 100 - add, 2, dclr, C.Black, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
							end
						else
							if activity == "ACT_VM_RELOAD_EMPTY" and self.NonEmptyReloadAnim then
					...
					if not self.WasZero then
						if self.dt.Sil and self.IncAmmoPercSil then
							if activity == "ACT_VM_RELOAD" and self.NonEmptyReloadAnim then
								...
								draw.BlurredText("RELOADING " .. math.Round(cyc / self.IncAmmoPercSil * 100) .. "%", "BROOP_Big", "BROOP_BigBlur", x - 100, y - 100 - add, 2, dclr, C.Black, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
							end
						else
							if activity == "ACT_VM_RELOAD" and self.NonEmptyReloadAnim then
								draw.BlurredText("RELOADING " .. math.Round(cyc * 100) .. "%", "BROOP_Big", "BROOP_BigBlur", x - 100, y - 100 - add, 2, dclr, C.Black, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
							else
								draw.BlurredText("RELOADING " .. math.Round(cyc / self.IncAmmoPerc * 100) .. "%", "BROOP_Big", "BROOP_BigBlur", x - 100, y - 100 - add, 2, dclr, C.Black, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
							end
						end
					else
						draw.BlurredText("RELOADING " .. clamp(math.Round(cyc * 100), 0, 100) .. "%", "BROOP_Big", "BROOP_BigBlur", x - 100, y - 100 - add, 2, dclr, C.Black, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
		...
		if GetConVarNumber("cstm_hud_reloadprogress") > 0 then
			...
			if activity == "ACT_VM_RELOAD" or activity == "ACT_VM_RELOAD_EMPTY" or activity == "ACT_VM_RELOAD_SILENCED" then
				if cyc < 0.99 then
					A = Lerp(FT * 10, A, 40)
		...
		if dtint3 == 19 then
			if not self.WeaponOverview then 
				...
				trect(x - 342, y - 348 - addy - add, 250, 190 + addy)
				...
				if self.ConeInaccuracyAff1 > self.ConeInaccuracyAff1 * maxsp then
				...
				elseif self.ConeInaccuracyAff1 < self.ConeInaccuracyAff1 * maxsp then
				...
				elseif self.Primary.Recoil * self.InaccAff1 < self.DefRecoil * self.InaccAff1 * incsp then
					draw.BlurredText("Spread per shot: +" .. math.Round(self.Primary.Recoil * self.InaccAff1 * 10 * incsp, 2) .. "%",  "BROOP_16", "BROOP_16Blur", x - 100, y - 183 - add, 1, C.Red, C.Black, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
				else
					draw.BlurredText("Spread per shot: +" .. math.Round(self.Primary.Recoil * self.InaccAff1 * 10 * incsp, 2) .. "%",  "BROOP_16", "BROOP_16Blur", x - 100, y - 183 - add, 1, C.White, C.Black, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
				end
...
if SERVER then
	function SWEP:RestoreStats()
		...
		if ps then
			...
			if ps.rec then
				self.DefRecoil = self.DefRecoil + self.Primary.Recoil * ps.rec
				self:SendRecoil()
			end
