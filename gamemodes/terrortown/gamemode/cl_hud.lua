...
local function PaintVBar(x, y, w, h, colors, value)
   -- Background
   -- We swap the color fill and background to simulate draining vertically... gmod hates me...
   draw.RoundedBox(8, x-1, y-1, w+2, h+2, colors.background)

   -- Fill
   local height = h * math.Clamp(value, 0, 1)

   if height > 0 then
      RoundedMeter(8, x, y, w, height, colors.fill)
   end
end
...
local function GetAmmo(ply)
   ...
   local alt_mode = weap.AltModeText
   local alt_num = weap.AltMode
   
   return ammo_clip, ammo_max, ammo_inv, alt_mode, alt_num
end
...
local function InfoPaint(client)
   ...
   if client:GetActiveWeapon().Primary then
      local ammo_clip, ammo_max, ammo_inv, alt_mode, alt_num = GetAmmo(client)
      if ammo_clip != -1 then
		 ...
		 if(alt_num != 99) then
			PaintBar(x+margin, (health_y - (bar_height*2) - (margin*4)), bar_width, bar_height, ammo_colors, 1)
			ShadowedText(alt_mode, "HealthAmmo", bar_width-200, (health_y- (bar_height*2) - (margin*4)), COLOR_WHITE, TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT)
		 end
      end
   end
   ...
   --- Stamina Bar --- 
	...
	if client:GetNWFloat("JetPack") != 0 then 
		local text = string.format("Fuel %i", client:GetNWFloat("JetFuel"))
		PaintBar(x+margin, (health_y - (bar_height*3) - (margin*6)), bar_width, bar_height,  ammo_colors, (client:GetNWFloat("JetFuel")/100))
		ShadowedText(text, "HealthAmmo", bar_width-200, (health_y- (bar_height*3) - (margin*6)), COLOR_WHITE, TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT)
	end
	

	if client:GetNetworkedFloat("FLASHED_END") > CurTime() then
		local FlashedEnd 	= client:GetNetworkedFloat("FLASHED_END")
		local FlashedStart 	= client:GetNetworkedFloat("FLASHED_START")
	...
	if client:GetNetworkedFloat("FLASHED_END")+2 > CurTime() then DrawMotionBlur( 0.1, 0.79, 0.05) end
	
	--- Jetpack fuel --
	
	--if client:GetNWFloat("JetPack") != 0 then 
	--	PaintVBar((width+margin+bar_height) , y, 25, 90,  ammo_colors, (client:GetNWFloat("JetFuel")/100))
	--end
   -- Draw traitor state
