...
KARMA.cv.max         = CreateConVar("ttt_karma_max", "2000")
...
KARMA.cv.kicklevel = CreateConVar("ttt_karma_low_amount", "24")
KARMA.cv.autoban   = CreateConVar("ttt_karma_low_ban", "1")
KARMA.cv.bantime   = CreateConVar("ttt_karma_low_ban_minutes", "10")
...
function KARMA.NotifyPlayer(ply)
   ...
   local k = math.Round(ply:GetBaseKarma()/10)
   if df > 0.99 then
...
function KARMA.Respawn( ply, wep, killer )
	timer.Simple(1,	function()
		if(GetRoundState() == ROUND_ACTIVE) then
			if not ply or not IsValid(ply) then
			    local karma = (ply:GetLiveKarma()/100)-10
			    local mathz = math.random(100)
				if karma < 0 then return end
				if mathz > karma then return end
				ulx.fancyLogAdmin( ply, false ,"The divines favored #A with a new life!" )
				timer.Simple(5,	function()
					local corpse = corpse_find(ply)
					if corpse then corpse_remove(corpse) end
					ply:SpawnForRound( true )
					ply:SetCredits( ( (ply:GetRole() == 0) and 0 ) or GetConVarNumber("ttt_credits_starting") )
				end)
			end
		end
	end)
end
hook.Add( "PlayerDeath", "KarmaRespawn", KARMA.Respawn );
