...
function ulx.karma( calling_ply, target_plys, amount )
	if not GetConVarString("gamemode") == "terrortown" then ULib.tsayError( calling_ply, gamemode_error, true ) else
    	for i=1, #target_plys do
			target_plys[ i ]:SetBaseKarma( amount*10 )
    	    target_plys[ i ]:SetLiveKarma( amount*10 )
    	end	
  	end
...
karma:addParam{ type=ULib.cmds.NumArg, min=0, max=200, default=1000, hint="Karma", ULib.cmds.optional, ULib.cmds.round }
