...
-- voteslay command by Shadow

local function voteSlayDone( t, target, ply, reason )
	local results = t.results
	local winner
	local winnernum = 0
	for id, numvotes in pairs( results ) do
	...
	if winner ~= 1 or winnernum < minVotes or winnernum / t.voters < ratioNeeded then
		...
		str = "Vote results: User will be slayed for the next two rounds. (" .. winnernum .. "/" .. t.voters .. ")"
		target:SetPData("slaynr_slays", 2)
		ulx.slaynr( ply, target, 2, 0 )
	end
...
function ulx.voteslay( calling_ply, target_ply, reason )
	...
	local msg = "Slay " .. target_ply:Nick() .. " for two rounds?"
	if reason and reason ~= "" then
		msg = msg .. " (" .. reason .. ")"
	end

	ulx.doVote( msg, { "Yes", "No" }, voteSlayDone, _, _, _, target_ply, calling_ply, reason )
	ulx.fancyLogAdmin( calling_ply, "#A started a vote to slay #T for 2 rounds ( .. reason .. )" , target_ply )
end
local voteslay = ulx.command( CATEGORY_NAME, "ulx voteslay", ulx.voteslay, "!voteslay" )
voteslay:addParam{ type=ULib.cmds.PlayerArg }
voteslay:addParam{ type=ULib.cmds.StringArg, hint="reason", ULib.cmds.optional, ULib.cmds.takeRestOfLine, completes=ulx.common_kick_reasons }
voteslay:defaultAccess( ULib.ACCESS_ALL )
voteslay:help( "Starts a public slay vote against target." )
if SERVER then ulx.convar( "voteslaySuccessratio", "0.6", _, ULib.ACCESS_ALL ) end -- The ratio needed for a voteslay to succeed
if SERVER then ulx.convar( "voteslayMinvotes", "2", _, ULib.ACCESS_ALL ) end -- Minimum votes needed for voteslay
